...
void*				KnobDriver_Create(CFAllocatorRef inAllocator, CFUUIDRef inRequestedTypeUUID);
...
static OSStatus	NullAudio_GetBoxPropertyData(AudioServerPlugInDriverRef inDriver, AudioObjectID inObjectID, pid_t inClientProcessID, const AudioObjectPropertyAddress* inAddress, UInt32 inQualifierDataSize, const void* inQualifierData, UInt32 inDataSize, UInt32* outDataSize, void* outData)
{
	...
	switch(inAddress->mSelector)
	{
		...
		case kAudioObjectPropertyModelName:
			//	This is the human readable name of the maker of the box.
			FailWithAction(inDataSize < sizeof(CFStringRef), theAnswer = kAudioHardwareBadPropertySizeError, Done, "NullAudio_GetBoxPropertyData: not enough space for the return value of kAudioObjectPropertyManufacturer for the box");
			*((CFStringRef*)outData) = CFSTR("Knob Model");
			*outDataSize = sizeof(CFStringRef);
			break;
...
static OSStatus	NullAudio_GetDevicePropertyData(AudioServerPlugInDriverRef inDriver, AudioObjectID inObjectID, pid_t inClientProcessID, const AudioObjectPropertyAddress* inAddress, UInt32 inQualifierDataSize, const void* inQualifierData, UInt32 inDataSize, UInt32* outDataSize, void* outData)
{
	...
	switch(inAddress->mSelector)
	{
		...
		case kAudioObjectPropertyName:
			...
			*((CFStringRef*)outData) = CFSTR("Knob");
			*outDataSize = sizeof(CFStringRef);
			break;
		...
		case kAudioObjectPropertyManufacturer:
			...
			*((CFStringRef*)outData) = CFSTR("duosonic62");
			*outDataSize = sizeof(CFStringRef);
			break;
		...
		case kAudioObjectPropertyOwnedObjects:
			...
			switch(inAddress->mScope)
			{
				case kAudioObjectPropertyScopeGlobal:
				{
					//	global scope means return all objects: both streams plus the volume and
					//	mute control for each side. The IDs are not contiguous (the data source
					//	controls were removed), so list them explicitly.
					static const AudioObjectID kGlobalOwnedObjects[] =
					{
						kObjectID_Stream_Input, kObjectID_Volume_Input_Master, kObjectID_Mute_Input_Master,
						kObjectID_Stream_Output, kObjectID_Volume_Output_Master, kObjectID_Mute_Output_Master
					};
					...
					{
						((AudioObjectID*)outData)[theItemIndex] = kGlobalOwnedObjects[theItemIndex];
					}
					break;
				}

				case kAudioObjectPropertyScopeInput:
					//	input scope means just the objects on the input side (stream, volume, mute)
					if(theNumberItemsToFetch > 3)
					{
						theNumberItemsToFetch = 3;
					}

					//	fill out the list with the right objects (input IDs are contiguous)
					for(theItemIndex = 0; theItemIndex < theNumberItemsToFetch; ++theItemIndex)
					{
				...
				case kAudioObjectPropertyScopeOutput:
					//	output scope means just the objects on the output side (stream, volume, mute)
					if(theNumberItemsToFetch > 3)
					{
						theNumberItemsToFetch = 3;
					}

					//	fill out the list with the right objects (output IDs are contiguous)
					for(theItemIndex = 0; theItemIndex < theNumberItemsToFetch; ++theItemIndex)
					{
		...
		case kAudioObjectPropertyControlList:
			...
			//	The control list is the volume and mute control for each side: the first two are
			//	the input volume and mute, the last two are the output volume and mute.
			for(theItemIndex = 0; theItemIndex < theNumberItemsToFetch; ++theItemIndex)
			{
				if(theItemIndex < 2)
				{
...
static OSStatus	NullAudio_GetStreamPropertyData(AudioServerPlugInDriverRef inDriver, AudioObjectID inObjectID, pid_t inClientProcessID, const AudioObjectPropertyAddress* inAddress, UInt32 inQualifierDataSize, const void* inQualifierData, UInt32 inDataSize, UInt32* outDataSize, void* outData)
{
	...
	switch(inAddress->mSelector)
	{
		...
		case kAudioObjectPropertyName:
			...
			*((CFStringRef*)outData) = (inObjectID == kObjectID_Stream_Input) ? CFSTR("Knob Input") : CFSTR("Knob Output");
			*outDataSize = sizeof(CFStringRef);
			break;
...
static OSStatus	NullAudio_DoIOOperation(AudioServerPlugInDriverRef inDriver, AudioObjectID inDeviceObjectID, AudioObjectID inStreamObjectID, UInt32 inClientID, UInt32 inOperationID, UInt32 inIOBufferFrameSize, const AudioServerPlugInIOCycleInfo* inIOCycleInfo, void* ioMainBuffer, void* ioSecondaryBuffer)
{
	//	This is called to actually perform a given operation. This device implements a passthrough
	//	loopback: audio written to the output stream (WriteMix) is stored in a ring buffer indexed by
	//	the output sample time, and reads from the input stream (ReadInput) return that audio indexed
	//	by the input sample time. The result is that whatever is played to the Knob output device is
	//	captured on the Knob input device (BlackHole-style loopback).
	...
	{
		//	Store the audio written to the output stream into the ring buffer, indexed by the
		//	output sample time. This is the "record" side of the loopback.
		...
		for(UInt32 theFrame = 0; theFrame < inIOBufferFrameSize; ++theFrame)
		{
			...
			gDevice_RingBuffer[(theRingFrame * 2) + 0] = theSource[(theFrame * 2) + 0];
			gDevice_RingBuffer[(theRingFrame * 2) + 1] = theSource[(theFrame * 2) + 1];
		}
	}
